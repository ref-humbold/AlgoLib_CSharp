...
namespace AlgoLib.Geometry.Dim2
{
    ...
    public class ClosestPointsTest
    {
        ...
        {
            // when
            (Point2D, Point2D) result = ClosestPoints.FindClosestPoints(new Point2D[] { Point2D.Of(2, 2) });
            // then
        ...
        public void FindClosestPoints_WhenTwoPoints_ThenThesePoints()
        {
            ...
            (Point2D, Point2D) result =
                    ClosestPoints.FindClosestPoints(new Point2D[] { Point2D.Of(2, 2), Point2D.Of(4, 4) });
            // then
        ...
        public void FindClosestPoints_WhenThreePoints_ThenPairOfClosestPoints()
        {
            // when
            (Point2D, Point2D) result =
                ClosestPoints.FindClosestPoints(new Point2D[] { Point2D.Of(3, 2), Point2D.Of(1, 1), Point2D.Of(7, 0) });
            // then
